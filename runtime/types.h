...
/* Forward declaration for pointers to image vectors. The actual structure is
 * defined in the dynamically generated part of the runtime.
 */
...
typedef void*    (*aktive_image_state_new)    (void* param, aktive_image_vector_ptr srcs);
typedef void     (*aktive_image_state_free)   (void* state);
...
typedef struct aktive_image_type {
    ...
    aktive_image_state_new     state_new   ; /* Create  custom image state        */
    aktive_image_state_free    state_free  ; /* Release custom image state        */
