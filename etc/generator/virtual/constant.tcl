...
operator image::from::matrix {
	...
	//
	//  a. The `.values.v` became already heap-allocated when it was copied into the
	...
	if (param->values.c < sz) {
	    param->values.v = REALLOC (param->values.v, double, sz);
	    memset (param->values.v + param->values.c, 0, sz*sizeof (double));
	}
	...
	state->src.used     = sz;
	state->src.pixel    = param->values.v;
